...
const Content = (props) => {
  ...
  const get = (page, dat, dat2, dat3) => {
    ...
    if (dat3.last_pay_time) {
      sort.push({ "last_pay_time": dat3.last_pay_time.replace("end", "") })
    }
    if (dat3.left_count) {
      sort.push({ "left_count": dat3.left_count.replace("end", "") })
    }
  ...
  StudyInfoExPage(query: ${JSON.stringify(JSON.stringify(query))}, page: 0, size: 10, sort: ${JSON.stringify(JSON.stringify(sort))}) {
    ...
    content {
      ...
      teacher_assign_time
        ...
        console.log(res)
        resolve({
          total: res.data.data.StudyInfoExPage ? res.data.data.StudyInfoExPage.totalElements : [],
          data: res.data.data.StudyInfoExPage ? res.data.data.StudyInfoExPage.content : 0,
          success: true
        })
      })
    })
      ...
      <ProTable
        columns={[
          ...
          },
          {
            ...
            sorter: (a, b) => parseInt(a.left_count) - parseInt(b.left_count),
